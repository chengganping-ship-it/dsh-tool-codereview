...
function detectMagicNumbers(code, lines, issues) {
    ...
    lines.forEach((line) => {
        if (/(const|let|var|final|val)\s+\w+\s*=/.test(line))
            return;
...
function analyzePerformance(code, language) {
    ...
    const inefficientLoopPatterns = [
        { pattern: /\.find\s*\(.*\.find\s*\(/, msg: 'Nested .find() calls - O(nÂ²) complexity' },
...
function formatReviewReport(result) {
    ...
    if (result.issues.length > 0) {
        ...
        result.issues.forEach(issue => {
            const icon = severityIcons[issue.severity] || 'âª';
...
function formatSecurityReport(result) {
    ...
    if (result.vulnerabilities.length > 0) {
        ...
        result.vulnerabilities.forEach(vuln => {
            const icon = vuln.severity === 'critical' ? 'ð´' : vuln.severity === 'high' ? 'ð ' : 'ð¡';
            ...
            lines.push(`  â *Fix: ${vuln.remediation}*`);
        });
        lines.push('');
    }
    else {
...
function formatDependencyReport(result) {
    ...
    if (result.dependencies.length > 0) {
        ...
        result.dependencies.forEach(dep => {
            const icon = dep.severity === 'critical' ? 'ð´' : dep.severity === 'high' ? 'ð ' : 'ð¡';
            const cveInfo = dep.cve ? ` [${dep.cve}]` : '';
            const fixedInfo = dep.fixedVersion ? ` â Upgrade to ${dep.fixedVersion}` : '';
...
function formatPerformanceReport(result) {
    ...
    if (result.issues.length > 0) {
        ...
        result.issues.forEach(issue => {
            const icon = issue.severity === 'warning' ? 'ð¡' : 'ðµ';
            const lineInfo = issue.line ? ` (line ${issue.line})` : '';
        ...
        async execute(args) {
            ...
            if (importantIssues.length > 0) {
                ...
                importantIssues.slice(0, 10).forEach(issue => {
                    const icon = issue.severity === 'critical' ? 'ð´' : issue.severity === 'error' ? 'ð ' : 'ð¡';
